...
#-------------------------------------------------------------
    ...
    def __createSearch(self, dic):
        ...
        for dictKey in dic.keys():
            ...
            i += 1
    def __pushJsonToJs(self):
    	path = myLib.getParentPath(2) + "/static/data/json/search_return.json"
    	print(path)
    	with open(path,'r') as f:
    		self.SigSendMessageToJS.emit(f.read())




    def on_btnConfirms_clickedd(self): 
